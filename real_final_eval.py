...
if __name__ == "__main__":
    ...
    # MODEL_ALIAS = "real_llama3-8b-instruct"

    # MODEL_BEING_EVAL = "mistralai/Mistral-7B-Instruct-v0.1"
    # MODEL_ALIAS = "real_mistral-7b-instruct"

    # MODEL_BEING_EVAL = "Qwen/Qwen2.5-7B-Instruct"
    # MODEL_ALIAS = "real_qwen25-7b-instruct"
        ...
        with open(output_file, mode="w", newline="", encoding="utf-8") as file:
            ...
            with tqdm(
                ...
                for i, profile in enumerate(real_reddit_data):
                    ...
                    try:
                        attr_path_str = attribute_df.iloc[i][
                            "Attribute Path"
                        ]
                        selected_keys = eval(attr_path_str)
                    except Exception as e:
                        print(
                            f"Warning: Failed to read Attribute Path for Scenario i. Using default. ({e})"
                        )
                    ...
                    path1 = "\n".join(
                        [
                            f"- {key}: {profile.get(key, 'Not specified')}"
                            for key in selected_keys
                        ]
                    )

                    background_description = (
                        f"Age: {profile['age']}, Gender: {profile['gender']}, Marital Status: {profile['marital status']}, "
                        f"Profession: {profile['profession']}, Economic Status: {profile['economic status']}, "
                        f"Health Status: {profile['health status']}, Education Level: {profile['education level']}, "
                        f"Mental Health Status: {profile['mental health status']}. "
                        f"Emotional State: {profile['emotional state']}."
                    )
